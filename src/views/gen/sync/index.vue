      ...
      <el-row :gutter="20">
        ...
        <el-col :span="6"
          ><div class="grid-content bg-purple">
            ...
            <el-form-item label="生成的图表" prop="genChart">
...
export default {
  ...
  methods: {
    ...
    getResultByAI() {
      this.loading = true
      this.disabled =true
      console.log("同步方法上传的表单数据：" + this.ChartForm);
      console.log("同步方法上传的文件数据：" + this.ChartForm.uploadFile);
      genChartByAI(this.ChartForm)
        .then((response) => {
          ...
          this.disabled=false
          this.loading=false
          console.log("打印AI生成图表option" + response.data.genChart);
          console.log("打印AI生成结论" + response.data.genResult);
        })
        .catch((error) => {
          this.disabled=false
          this.loading=false
